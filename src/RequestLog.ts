...
const route_logging = new LoggingBase({ name: "access", errorfile: null, console_out: config.general.dev })
const RequestLog = async (ctx: Context, next) => {
   if (!config.web.access_log) return next();
   ...
   let print = () => {
      ...
      let m = ctx.method
      while (m.length < 4) m += " "
      let message = `${m} ${ctx.originalUrl} ${resColor}${status}\x1b[0m - ${time}ms`;
